...
import com.doerapispring.domain.{DeprecatedTodoListModel, ListId, TodoId}
...
class TodoListModelResourceTransformerTest {
  ...
  final private val todoListModel = new DeprecatedTodoListModel(new ListId(listId), profileName, List(), Date.from(Instant.EPOCH), 0, sectionName, deferredSectionName)
  ...
  def show_returnsList(): Unit = {
    ...
    val todoListModel = DeprecatedTodoListModel.add(this.todoListModel, todoId, task)
      .flatMap(todoListModel => DeprecatedTodoListModel.addDeferred(todoListModel, deferredTodoId, deferredTask))
      .flatMap(todoListModel => DeprecatedTodoListModel.unlock(todoListModel, Date.from(Instant.now())))
      .get
    ...
    def show_returnsListUnlockDuration(): Unit = {
      ...
      val todoList = DeprecatedTodoListModel.unlock(this.todoListModel, Date.from(currentInstant.minusMillis(delta))).get
  ...
  @Test def show_returnsList_includesLinksForEachTodo(): Unit = {
    val todoListModel = DeprecatedTodoListModel.add(this.todoListModel, new TodoId("twoNowId"), "twoNowTask")
      .flatMap(todoListModel => DeprecatedTodoListModel.add(todoListModel, new TodoId("oneNowId"), "oneNowTask"))
      .flatMap(todoListModel => DeprecatedTodoListModel.addDeferred(todoListModel, new TodoId("oneLaterId"), "oneLaterTask"))
      .flatMap(todoListModel => DeprecatedTodoListModel.addDeferred(todoListModel, new TodoId("twoLaterId"), "twoLaterTask"))
      .flatMap(todoListModel => DeprecatedTodoListModel.unlock(todoListModel, Date.from(Instant.now())))
      .get
    ...
    @Test def show_whenListIsAbleToBeReplenished_includesPullLink(): Unit = {
      val todoListModel = DeprecatedTodoListModel.addDeferred(this.todoListModel, new TodoId("someTodoId"), "someTask").get
      val responseEntity = todoListReadModelResourceTransformer.transform(todoListModel, Date.from(Instant.now()))
    ...
    @Test def show_whenListIsNotAbleToBeEscalated_doesNotIncludeEscalateLink(): Unit = {
      val responseEntity = todoListReadModelResourceTransformer.transform(todoListModel, Date.from(Instant.now()))
      assertThat(DeprecatedTodoListModel.escalateCapability(todoListModel).isFailure).isTrue
