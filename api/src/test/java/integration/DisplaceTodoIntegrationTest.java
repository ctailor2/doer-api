...
public class DisplaceTodoIntegrationTest extends AbstractWebAppJUnit4SpringContextTests {
    ...
    public void displace_replacesImmediatelyScheduledTodo_bumpsItToPostponedList() throws Exception {
        ...
        nextActionHref = JsonPath.parse(mockMvc.perform(post(nextActionHref)
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .headers(httpHeaders))
                .andReturn().getResponse().getContentAsString()).read("$.list._links.displace.href", String.class);

        mockMvc.perform(post(nextActionHref)
            .content("{\"task\":\"do the things\"}")
            .contentType(MediaType.APPLICATION_JSON)
            .headers(httpHeaders))
                .andExpect(jsonPath("$.list.todos[*].task", contains("do the things", "some task")))
                .andExpect(jsonPath("$.list.deferredTodos[*].task", contains("some other task")));
    }
}
